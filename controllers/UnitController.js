...
exports.index = async (req, res) => {
    try {
        ...
        const totalQuery = await db('unit').count('* as count');
        const total = totalQuery[0].count;

        let query = db('unit').select('*').orderBy('id', 'desc');
