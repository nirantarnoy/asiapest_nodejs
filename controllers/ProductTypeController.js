...
exports.index = async (req, res) => {
    try {
        ...
        const totalQuery = await db('product_type').count('* as count');
        const total = totalQuery[0].count;

        let query = db('product_type').select('*').orderBy('id', 'desc');
...
exports.edit = async (req, res) => {
    ...
    const { code, name, description } = req.body;
    try {
        await db('product_type').where({ id }).update({
            code,
            name,
            description,
...
exports.delete = async (req, res) => {
    const { id } = req.params;
    try {
        await db('product_type').where({ id }).delete();
        res.redirect('/product-types');
    } catch (err) {
        console.error(err);
